...
@pytest.mark.parametrize(
    ...
    [
        ("prot-1933--fk--5.xml", 1, 1, cluster.MergeStrategyType.chain),
        ...
        ("prot-1955--ak--22.xml", 147, 147, cluster.MergeStrategyType.chain),
        ...
        ('prot-199192--127.xml', 222, 222, cluster.MergeStrategyType.chain),
        ('prot-199192--127.xml', 49, 49, cluster.MergeStrategyType.who),
        ('prot-199192--127.xml', 208, 208, cluster.MergeStrategyType.who_sequence),
        ('prot-199192--127.xml', 208, 208, cluster.MergeStrategyType.who_speaker_hash_sequence),
...
def test_protocol_to_speeches_with_different_strategies(
    ...
    speeches = cluster.to_speeches(protocol=protocol, merge_strategy=strategy, skip_size=1)
    ...
    speeches = cluster.to_speeches(protocol=protocol, merge_strategy=strategy, skip_size=3)
    assert all(len(x.text) >= 3 for x in speeches)

    speeches = cluster.to_speeches(protocol=protocol, merge_strategy=strategy, skip_size=100)
...
def test_to_speeches_with_faulty_attribute(filename, expected_speech_count):
    ...
    speeches = cluster.to_speeches(protocol=protocol, merge_strategy=cluster.MergeStrategyType.chain)
...
def test_protocol_to_speeches(protocol_name: str):
    ...
    for merge_strategy in ['who_sequence', 'who_speaker_hash_sequence', 'speaker_hash_sequence', 'chain']:
        ...
        merger: cluster.IMergeStrategy = cluster.MergerFactory.get(merge_strategy)
