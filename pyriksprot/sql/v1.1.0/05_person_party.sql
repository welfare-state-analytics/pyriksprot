...
insert into person_party (
    ...
    with affiliation as (
        select
            ...
            cast(substr(cast(_party_affiliation."start" as text), 1, 4) as integer) as "start_year",
            cast(substr(cast(_party_affiliation."end" as text), 1, 4) as integer)   as "end_year"
        from _party_affiliation
    )
        select
            row_number() over (order by affiliation."person_id" asc),
